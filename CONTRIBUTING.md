...
### 0.1. Simulate stronghold generation logic

To get exact coordinates of silverfish spawner blocks you need to simulate stronghold generation. This would remove annoying process of manual seed filtration.

### 0.2. More versions

Loop though all versions where stronghold or biome generation changed.
...
To fork this repository simply [click "Fork" button](https://github.com/KK-mp4/double-silverfish-spawner-finder/fork).

```bash
# 1. Fork via GitHub UI
# 2. Clone your fork locally (replace your-username)
git clone git@github.com:your-username/double-silverfish-spawner-finder.git
cd double-silverfish-spawner-finder

# Add upstream remote (original repo)
git remote add upstream git@github.com:KK-mp4/double-silverfish-spawner-finder.git
git fetch upstream
```
...
```text
└───double-silverfish-spawner-finder
    │   CMakeLists.txt
    │
    ...
    ├───include - helper functions like config
    ├───logs - finders output
    └───src - main seed finder source code
            main.cpp - entry point
            stronghold_finder.cpp - seed finder itself
```
